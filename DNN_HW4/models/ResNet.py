...
class ResNet(nn.Module):
    def __init__(self, input_channel, output_dim, learning_rate, reg_lambda, device):
        ...
        # =============================== EDIT HERE ===============================
        self.CONV2_x = nn.Sequential(BasicBlock(in_channels=64, out_channels=64), BasicBlock(in_channels=64, out_channels=64))

        self.CONV3_x = nn.Sequential(BasicBlock(in_channels=64, out_channels=128, stride=2), BasicBlock(in_channels=128, out_channels=128))

        self.CONV4_x = nn.Sequential(BasicBlock(in_channels=128, out_channels=256, stride=2), BasicBlock(in_channels=256, out_channels=256))

        self.CONV5_x = nn.Sequential(BasicBlock(in_channels=256, out_channels=512, stride=2), BasicBlock(in_channels=512, out_channels=512))
        
        self.POOL2 = nn.AvgPool2d(kernel_size=(7,7), stride=2)
    
        self.FC1 = nn.Linear(512, output_dim)

        self.Conv_layers = nn.Sequential(self.CONV1, nn.ReLU(), self.POOL1, self.CONV2_x, self.CONV3_x, self.CONV4_x, self.CONV5_x, self.POOL2)
